...
Partner bounds default to the frozen manuscript comparison in `results/paper_v1_1_snapshot.json`; `--known-bounds FILE` supplies a different explicit source. The verifier does not infer exact unknotting numbers from certificate assertions. The scripts use exact combinatorial replay and knot Floer genus detection, together with numerical SnapPy isometries for named hyperbolic knots. These are reproducibility checks, not formal proof certificates for the numerical isometry step.
...
| `bj_scan.py` | Examine crossing neighbors of prime alternating knots for the conditional Bernhard–Jablan analysis in Section 4 |
