...
These scripts support Section 5.1 of **Computation of Unknotting Numbers: Which Knot Breaks the
Bernhard–Jablan Conjecture?**. They apply the half-integral surgery obstruction to
...
unknotting number is two. Their theorem already established the weak
Bernhard–Jablan counterexample unconditionally; the new computations
identify this knot as a counterexample to the original, strong form. The upper bounds and the exhaustive minimal-diagram
...
spectral sequence used to prove the L-space premise. Its filtered Floer
complex has the absolute Maslov grading of Theorem 4.1; the differential
lowers that grading by one and respects the Spin^c decomposition, so the
induced pages retain those gradings. In an L-space, the unique
...
identifications considered. For odd determinant `D`, each candidate vector
is tested under `2 * phi(D) * D` affine maps and orientations: 6,072 tests
for `D = 69`, and 1,320 tests for each of the nine candidate vectors for
`D = 33` (11,880 in total). No choice of a compatible surgery structure means
